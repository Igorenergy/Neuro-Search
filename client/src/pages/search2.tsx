    ...
    <div className="max-w-7xl mx-auto font-sans text-gray-800">
      ...
      <div className="mb-8 space-y-6">
        <div className="flex justify-between items-center">
          <h1 className="text-xl font-bold text-gray-900">Search in researches</h1>
          <div className="flex items-center gap-2 text-sm text-gray-500">
            <Info className="w-4 h-4" />
            <span>Looking for specific documents or media? Search in the <a href="#" className="text-[#008DA8] underline decoration-[#008DA8] underline-offset-2 font-medium">Assets Repository</a></span>
          </div>
        </div>

        {/* Search Hero */}
        <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200 space-y-4">
          <label className="text-sm font-semibold text-gray-700 block">Search query</label>
          <div className="relative">
             ...
             />
             <Button variant="ghost" size="icon" className="absolute right-2 top-1/2 -translate-y-1/2 hover:bg-transparent">
               <Search className="w-6 h-6 text-black stroke-[3]" />
             </Button>
          </div>
          <div className="flex items-center gap-2 text-sm text-gray-500">
            <span>Latest queries:</span>
            {["[Inflation 2025]", "[Crypto Risks]", "[Biotech trends]"].map((tag, i) => (
              <span key={i} className="text-gray-800 font-medium cursor-pointer hover:text-[#008DA8] hover:underline">
                {tag}{i < 2 ? "," : ""}
              </span>
            ))}
          </div>
        ...
        {isEmpty ? (
           /* No Dead Ends State */
            <div className="flex flex-col items-center justify-center py-20 animate-in fade-in duration-300">
              ...
              <p className="text-gray-500 mb-8 text-center max-w-md">
                However, we found <span className="font-bold text-gray-900">3 matching files</span> in the Assets Repository.
              </p>
          ...
          <div>
            ...
            <div className="divide-y divide-gray-100">
              {results.map((item) => (
                <div key={item.id} className="grid grid-cols-[1fr_120px_120px_150px_40px] gap-4 px-6 py-4 items-center hover:bg-gray-50 transition-colors group">
